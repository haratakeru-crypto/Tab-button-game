...
const POWERPOINT_BUTTON_QUESTIONS_FILE_PATH = path.join(process.cwd(), "data", "powerpoint-button-questions.json");

export async function GET() {
  try {
    const fileContents = await fs.readFile(POWERPOINT_BUTTON_QUESTIONS_FILE_PATH, "utf8");
    const questions: Question[] = JSON.parse(fileContents);
    return NextResponse.json({ questions });
  } catch (error) {
    console.error("Error reading powerpoint button questions:", error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    await fs.writeFile(POWERPOINT_BUTTON_QUESTIONS_FILE_PATH, JSON.stringify(questions, null, 2), "utf8");

    return NextResponse.json({ success: true, question: questions[idx] });
  } catch (error) {
    console.error("Error updating powerpoint button questions:", error);
    return NextResponse.json({ error: "サーバーエラーが発生しました" }, { status: 500 });
  }
}
